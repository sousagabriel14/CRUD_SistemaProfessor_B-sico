package br.edu.unicid.web;

import java.io.IOException;
import java.util.List;
import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import br.edu.unicid.bean.Professor;
import br.edu.unicid.dao.ProfessorDAO;

@WebServlet("/consultar.professor")
public class ServletProfessorConsultar extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
	public ServletProfessorConsultar() {
		super();
	} 
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException {
		doPost(request, response);
	}
	
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException {
		try {			
			ProfessorDAO professorDAO = new ProfessorDAO();
			List<Professor> professores = professorDAO.listarProfessorLike(request.getParameter("nomeProfessor"));	
		
			request.setAttribute("professor", professores);
			
			RequestDispatcher rd = request.getRequestDispatcher("consultar.jsp"); 
			rd.forward(request, response); 
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
}